...
mod tests {
    ...
    /// 构造一个 CRC 正确的完整 AWS Event Stream 帧。
    ///
    /// 布局:total_len(4) + header_len(4) + prelude_crc(4) + headers + payload + msg_crc(4)
    fn build_frame(headers_raw: &[u8], payload: &[u8]) -> Vec<u8> {
        let header_len = headers_raw.len() as u32;
        let total_len = (PRELUDE_SIZE + headers_raw.len() + payload.len() + 4) as u32;
        ...
        let msg_crc = crc32(&msg); // 对不含最后 4 字节的整条消息
        msg.extend_from_slice(&msg_crc.to_be_bytes());
        msg
    }

    /// 端到端正路:喂一个完整帧,解出 header(:event-type) + payload JSON。
    /// 验证搬运后整条链 decoder→frame→header→crc 贯通(搬运后补的冒烟)。
    #[test]
    fn test_decode_full_frame_happy_path() {
        // header: name_len(1)=":event-type"(11) + type(7=String) + val_len(2) + "assistantResponseEvent"
        let name = b":event-type";
        let val = b"assistantResponseEvent";
        let mut headers_raw = Vec::new();
        headers_raw.push(name.len() as u8);
        headers_raw.extend_from_slice(name);
        headers_raw.push(7u8); // String
        headers_raw.extend_from_slice(&(val.len() as u16).to_be_bytes());
        headers_raw.extend_from_slice(val);

        let payload = br#"{"content":"hello"}"#;
        let frame_bytes = build_frame(&headers_raw, payload);

        let mut decoder = EventStreamDecoder::new();
        decoder.feed(&frame_bytes).unwrap();
        let frame = decoder.decode().unwrap().expect("应解出一帧");
        assert_eq!(frame.event_type(), Some("assistantResponseEvent"));
        let json: serde_json::Value = frame.payload_as_json().unwrap();
        assert_eq!(json["content"], "hello");
        // 帧已消费完,再 decode 应为 None
        assert!(matches!(decoder.decode(), Ok(None)));
    }

    /// 分片喂入:把一个完整帧拆成两半分别 feed,decoder 应在凑齐后才解出。
    #[test]
    fn test_decode_split_across_feeds() {
        ...
        headers_raw.push(name.len() as u8);
        headers_raw.extend_from_slice(name);
        headers_raw.push(7u8);
        headers_raw.extend_from_slice(&(val.len() as u16).to_be_bytes());
        headers_raw.extend_from_slice(val);
        let frame_bytes = build_frame(&headers_raw, b"{}");

        let mid = frame_bytes.len() / 2;
        let mut decoder = EventStreamDecoder::new();
        decoder.feed(&frame_bytes[..mid]).unwrap();
        assert!(matches!(decoder.decode(), Ok(None)), "半包应返回 None");
        decoder.feed(&frame_bytes[mid..]).unwrap();
        let frame = decoder.decode().unwrap().expect("凑齐后应解出");
        assert_eq!(frame.event_type(), Some("x"));
    }
}
