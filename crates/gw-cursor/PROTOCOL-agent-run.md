...
⚠️ **先说清口径**（`chat.rs` 的 `to_anthropic_usage` 注释也写了）：
`input_tokens` 是**总上下文**，`cache_read_input_tokens` 是**它的子集**。
所以命中率 = `cache_read / input_tokens`，**不是** `cache_read / (input + cache_read)`
—— 后者把缓存部分在分母里重复计了一次，会把上限压到 50% 附近。
真正要看的成本项是 **`input_tokens - cache_read`（本轮新算的 token）**。

`system` ≈800 字 + 4 轮对话，同一 `x-session-id`：

| | input（总） | cache_read | **新算** | 命中率 |
|---|---|---|---|---|
| 轮1 | 12951 | 12800 | **151** | 98.8% |
| 轮2 | 12987 | 12928 | **59** | 99.5% |
...
| | input（总） | cache_read | **新算** | 命中率 |
|---|---|---|---|---|
| 轮1 | 13039 | 256 | **12783** | 2.0% |
| 轮2 | 13072 | 8320 | **4752** | 63.6% |
| 轮3 | 13095 | 8320 | **4775** | 63.5% |
| 轮4 | 13118 | 12928 | **190** | 98.6% |

**收益是「新算 token」从每轮上万降到两位数**，不是 input_tokens 变小 ——
后者是总上下文，历史在服务端也照样计入总数，只是绝大部分按缓存价结算。

另有两条排查时会踩的坑：

1. **`input_tokens` 不随轮次下降是正常的**，别拿它判断有状态生不生效。
   要看 `input - cache_read`。
2. **上游缓存跨会话共享**：无状态轮1 只有 2.0%，而有状态轮1 有 98.8% ——
   同一段 `system` 已被前一次实验喂进去了。所以**第一轮的数字受历史污染**，
   不能当冷启动基线。想测真冷启动得换一段没用过的 system 文本。

（真客户端 turn1 98858B → turn2 2121B 的 47 倍差是**上传字节数**，
与 token 计费是两件事，别混着看。）
